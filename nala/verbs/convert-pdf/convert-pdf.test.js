...
const pdfFilePath = path.resolve(__dirname, '../../assets/1-PDF-convert-pdf.pdf');
...
test.describe('Unity Convert PDF test suite', () => {
  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('Verify global nav (smoke) and breadcrumbs', async () => {
      await convertPdf.gnav.waitFor({ state: 'visible' });
      await expect(convertPdf.gnav).toBeVisible();
      await expect(convertPdf.gnavBreadcrumbs).toBeVisible();
    });

    await test.step('Verify Convert PDF widget content/specs', async () => {
      await expect(convertPdf.widget).toBeVisible();
    ...
    await test.step('Verify how-to section', async () => {
      await convertPdf.howToSection.scrollIntoViewIfNeeded();
      await expect(convertPdf.howToSection).toBeVisible({ timeout: 60000 });
    });

    await test.step('Verify three-up section', async () => {
      await convertPdf.threeUpSection.scrollIntoViewIfNeeded();
      await expect(convertPdf.threeUpSection).toBeVisible({ timeout: 60000 });
    });

    await test.step('Verify FAQ accordion', async () => {
      const { faqSection, faqAccordionTriggers } = convertPdf;
    ...
    await test.step('Verify ratings and reviews (RnR) block', async () => {
      await convertPdf.rnrSection.scrollIntoViewIfNeeded();
      await expect(convertPdf.rnrSection).toBeVisible({ timeout: 60000 });
      await expect(convertPdf.rnrSection.locator('.rnr-container')).toBeVisible({ timeout: 60000 });
    });

    await test.step('Verify columns section has 31 links', async () => {
      const { columnsSection, columnsATags } = convertPdf;
    ...
    await test.step('Verify footer', async () => {
      await convertPdf.footer.scrollIntoViewIfNeeded();
      await expect(convertPdf.footer).toBeVisible({ timeout: 60000 });
    });
