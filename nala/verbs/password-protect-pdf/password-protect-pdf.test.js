...
test.describe('Unity Password Protect PDF test suite', () => {
  ...
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL, browserName }) => {
    ...
    await test.step('Verify Password Protect PDF widget content/specs', async () => {
      ...
      await expect(passwordProtectPdf.verbHeader).toBeVisible();
      await expect(passwordProtectPdf.verbTitle).toBeVisible();
      await expect(passwordProtectPdf.verbCopy).toBeVisible();
