...
const excelToPdfFilePath = path.resolve(__dirname, '../../../assets/1-XLSX-excel-to-pdf.xlsx');

let excelToPdf;

const unityLibs = process.env.UNITY_LIBS || '';

test.describe('Unity Excel to PDF converter test suite', () => {
  test.beforeEach(async ({ page }) => {
    excelToPdf = new ExcelToPdf(page);
  });

  // Test 0 : Excel to PDF converter
  test(`${features[0].name},${features[0].tags}`, async ({ page, baseURL }) => {
    console.info(`[Test Page]: ${baseURL}${features[0].path}${unityLibs}`);
    const { data } = features[0];

    await test.step('step-1: Go to Excel to PDF converter test page', async () => {
      await page.goto(`${baseURL}${features[0].path}${unityLibs}`);
      await page.waitForLoadState('domcontentloaded');
      await expect(page).toHaveURL(`${baseURL}${features[0].path}${unityLibs}`);
    });

    await test.step('step-2: Verify Excel to PDF converter content/specs', async () => {
    ...
    await test.step('step-3: Upload a Excel file to convert to PDF', async () => {
      // upload and wait for some page change indicator (like a new element or URL change)
      const fileInput = page.locator('input[type="file"]#file-upload');
      await page.waitForTimeout(15000);
      console.log(`[Excel File Path]: ${excelToPdfFilePath}`);
      await fileInput.setInputFiles(excelToPdfFilePath);
      await page.waitForTimeout(15000);
