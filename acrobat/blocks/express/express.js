...
window.addEventListener('analyticsLoad', async ({ detail }) => {
  ...
  if (
    ...
    window.lana?.log(
      'Analytics failed to initialize correctly: some methods remain no-ops on express block',
      lanaOptions,
    );
  }
});
...
export default async function init(element) {
  ...
  const container = createTag('div', { class: 'express-container' });
  const row = createTag('div', { class: 'express-row' });
  const leftCol = createTag('div', { class: 'express-col express-col-left' });
  const rightCol = createTag('div', { class: 'express-col express-col-right' });
  const header = createTag('div', { class: 'express-header' });
  if (authoredSvg) {
    const svgImg = createTag('img', {
      src: authoredSvg.url,
      alt: authoredSvg.altText,
      class: 'express-title-svg',
    });
    ...
    const title = createTag('div', { class: 'express-title' });
  ...
  const headingEl = createTag('h1', { class: 'express-heading' }, heading);
  ...
  const subCopies = ['', '-2']
    .map((suffix) => {
      ...
      const el = createTag('p', { class: 'express-copy-sub' });
      const icon = createSvgElement('SUBCOPY_CHECK');
      if (icon) {
        icon.classList.add('express-copy-sub-icon');
        icon.setAttribute('aria-hidden', 'true');
        el.appendChild(icon);
      }
      el.appendChild(createTag('span', { class: 'express-copy-sub-label' }, subCopyText));
      return el;
    })
    .filter(Boolean);
  const copy1 = createTag('p', { class: 'express-copy' }, copy1Text);
  const dropzone = createTag('div', {
    class: 'express-dropzone',
    id: 'drop-zone',
  });
  ...
  const ctaLabel = createTag('span', { class: 'express-cta-label' }, ctaButtonLabel);
  ctaButton.appendChild(ctaLabel);
  const dragText = createTag('p', { class: 'express-drag' }, window.mph?.[`verb-widget-${VERB}-dragndrop-text`] || '');
  const fileLimitText = createTag('p', {
    class: 'express-file-limit',
  ...
  const footer = createTag('div', { class: 'express-footer' });
  const { locale } = getConfig();
  ...
  const legalText = createTag('p', { class: 'express-legal' }, legalInitial);
  ...
  if (!omitFooterForMobileStore && !(limits?.mobileApp && isMobile)) {
    if (legalText.textContent) {
      const createLegalLink = (label, url) => `<a class="express-legal-url" target="_blank" href="${url}">${label}</a>`;
      const legalLinks = [
  ...
  if (media) {
    const mediaWrapper = createTag('div', { class: 'express-media' });
  ...
  // Page-wide drop overlay — covers the full viewport during drag
  const pageOverlay = createTag('div', { class: 'express-page-overlay', 'aria-hidden': 'true' });
  ...
  const setDraggingClass = (shouldToggle) => {
    ...
    element.classList.toggle('dragging-block', !!shouldToggle);
  };
  let outsideClickHandler = null;
  const closeError = () => {
    errorState.classList.remove('express-error');
  ...
  const handleError = (detail, logToLana = false, logOptions = {}) => {
    ...
    if (message) {
      ...
      errorState.classList.add('express-error');
  ...
  if (useFileUpload && fileInput) {
    ...
    // Page-wide drag-and-drop: listen on document so any area of the page is a valid drop target
    let reDispatchingDrop = false;
    ...
    document.addEventListener('drop', (e) => {
      ...
      if (files.length > 0) {
        noOfFiles = files.length;
        // If the drop landed outside the block, forward it to the dropzone so Unity handles it
        if (!element.contains(e.target)) {
          reDispatchingDrop = true;
  ...
  function soloUpload() {
    ...
    uploadLinks.forEach((link) => {
      const labelElement = createTag('label', {
        for: 'file-upload',
        class: 'express-cta express-cta-solo',
        tabindex: 0,
  ...
  if (!window.adobeIMS?.isSignedInUser?.() && isLimitExhausted) {
    ...
    try {
      ...
      window.lana?.log(
        `Error Code: Unknown, Status: 'Unknown', Message: verb-widget init from express failed: ${error.message}`,
        lanaOptions,
      );
      widgetRoot.remove();
    }
