...
function isClientFile(file, limits) {
...
export function validateFiles(files, verb) {
  ...
  for (const file of files) {
    ...
    const clientFile = isClientFile(file, limits);
    if (limits.clientAcceptedFiles?.includes(ext) && !clientFile) {
      return {
        valid: false,
        code: 'error_unsupported_type',
        message: mph['verb-widget-error-unsupported-type'] || 'This file is in a format not supported for conversion to PDF.',
    ...
    const maxFileSize = clientFile ? limits.clientMaxFileSize : limits.maxFileSize;
...
let unityModulePromise = null;
function warmUnity() {
  if (!unityModulePromise) unityModulePromise = import('../unity/unity.js');
  return unityModulePromise;
}
...
export default async function init(element) {
  ...
  let unityInitPromise = null;

  function wireUnityEvents() {
    ...
    element.addEventListener('unity:show-error-toast', (e) => {
      const { code, message, info, metaData, errorData } = e.detail || {};
      if (!code) return;
      isUploading = false;
      ctaButton.disabled = false;
      ctaButton.querySelector('.verb-cta-label').textContent = ctaLabel;
      if (message) showError(message);
      window.lana?.log(`Error Code: ${code}, Message: ${message}, Info: ${info}`, lanaOptions);
      if (code.includes('cookie_not_set')) return;
      const analyticsKey = Object.keys(errorAnalyticsMap).find((k) => code.includes(k));
      if (analyticsKey) {
        const event = errorAnalyticsMap[analyticsKey];
        window.analytics.verbAnalytics(event, VERB, event === 'error' ? { errorInfo: info } : {});
      }
      const splunkData = { ...metaData, errorData };
      window.analytics.sendAnalyticsToSplunk(analyticsKey, VERB, splunkData, getSplunkEndpoint());
      exitFlag = true;
    });
  }
